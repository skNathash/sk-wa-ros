...
const SkReceivePackageModal: React.FC<SkReceivePackageModalProps> = ({
  show,
  boxNo,
  callback,
  orderId,
    ...
    const fetchPackageDetails = async () => {
      try {
        ...
        const dealIds: string[] = items
          .map((e: any) => e.dealId || e.dealRefId)
          .filter(Boolean);
        ...
        setProducts(
          items.map((e: any) => {
            ...
            return {
              ...e,
              // normalize names used by Desktop/Mobile components
              dealId: e.dealId,
              dealRefId: e.dealRefId,
              dealName: e.name || e.dealName || e.productName || "",
              packageQuantity:
                e.quantity || e.packageQuantity || e.packages?.quantity || 0,
              mrp: e.mrp || e.dealMrp || (e.packages && e.packages.mrp) || 0,
              price: e.price || e.purchasePrice || e.dealB2bPrice || 0,
    ...
    if (a.action === "update") {
      setProducts(
        produce((draft) => {
          ...
          const orderedQty = Number(product.packages?.quantity) || 0;
  ...
  const submitReceive = async () => {
    ...
    const payload: any = {
      invoiceId: boxDetails?.actualInvoiceNo || "",
      packageId: boxNo || "",
      items: (products || []).map((e: any) => ({
        ...
        invoiceQuantity:
          Number(e.packageQuantity) ||
          Number(e.quantity) ||
          Number(e.packages?.quantity) ||
          0,
        damagedQuantity: Number(e.damagedQty) || 0,
        damagedImages: Array.isArray(e.snapshots)
          ? e.snapshots
          : Array.isArray(e.images)
          ? e.images
          : [],
        shortageQuantity: Number(e.shortageQty) || 0,
        mrp: Number(e.mrp || e.dealMrp || (e.packages && e.packages.mrp) || 0),
        purchasePrice: Number(
          e.price || e.purchasePrice || e.dealB2bPrice || e.packageB2bPrice || 0
        ),
        tax: Number(e.tax || 0),
...
export default SkReceivePackageModal;
