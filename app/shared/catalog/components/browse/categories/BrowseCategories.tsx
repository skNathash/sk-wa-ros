...
const BrowseCategories: React.FC<{
  ...
  useEffect(() => {
    ...
    if (brandId) {
      filterRef.current = { ...filterRef.current, brandId };
    }
  ...
  }, [applyFilter, dependsOn, menuId, brandId]);
  ...
  // no local selection state - parent controls selection via `selectedId` prop
  ...
  const handleBrowseCallback = useCallback(
    (p: { action: string; data?: any }) => {
      if (p.action === "search") {
        ...
        // debounce search to avoid firing applyFilter on every keystroke
    ...
    return (
      <div className="tw:flex tw:justify-center tw:items-center tw:h-[calc(100vh-200px)] tw:bg-white tw:rounded-md tw:border tw:border-gray-200">
        <div className="tw:text-gray-500 tw:text-sm tw:text-center">
          Please select a {dependsOn?.join(" or ")} to view categories.
        </div>
      </div>
    );
  }
  ...
  return (
    <BrowseContainer
      ...
      title={title}
      subtitle={`${paginationRef.current.totalRecords} ${t("categories")}`}
      isLoading={loading}
      search={search}
      alpha={alpha}
    >
      {items.map((it) => (
        <Item
          ...
          selected={selectedId != null && selectedId === it._id}
