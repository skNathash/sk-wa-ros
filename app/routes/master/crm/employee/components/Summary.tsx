...
import { OVERDUE_STATUS, type EmployeeSummary } from "../helper";
...
type Tile = {
  ...
  // The status value this card filters by when clicked. Undefined = a
  // context-only tile (a metric with no follow-up status to filter by).
  status?: string;
};
    ...
    >
      <div className="tw:grid tw:grid-cols-2 tw:sm:grid-cols-4 tw:divide-x tw:divide-y tw:sm:divide-y-0 tw:divide-gray-100">
        {tiles.map(({ key, label, color, Icon, status }) => {
          ...
          // Context-only tiles (no status) aren't filterable.
          const clickable = !!onSelect && status !== undefined;
          const active = status !== undefined && activeStatus === status;
          return (
            <button
              key={key}
              type="button"
              onClick={clickable ? () => onSelect?.(status as string) : undefined}
              disabled={!clickable}
