...
// import MasterEmployeeSearchInput from "~/shared/crm/components/search-input/master-employee/MasterEmployeeSearchInput";
import type { DashboardFilterForm } from "./helper";
import { defaultFilter } from "./helper";

interface FilterProps {
  // Fired whenever a field changes so the parent can sync query params
  callback: (formData: DashboardFilterForm) => void;
  className?: string;
}
...
const Filter = ({ callback, className }: FilterProps) => {
  const { control, register, getValues, setValue, reset } =
    useFormContext<DashboardFilterForm>();

  const [statesOption, setStatesOption] = useState<Option[]>([]);
  const [districtsOption, setDistrictsOption] = useState<Option[]>([]);

  const [state, district, employeeId, employeeName] = useWatch({
    control,
    name: ["state", "district", "employeeId", "employeeName"],
  });
  ...
  // Selecting an employee scopes the list by their record id (employeeId);
  // clearing the selection removes the scope.
  const handleEmployeeSelect = (item: any, action: "add" | "remove") => {
    if (action === "remove") {
      setValue("employeeId", "");
      setValue("employeeName", "");
    } else {
      setValue("employeeId", item?.value?.refId || item?.value?.id || "");
      setValue("employeeName", item?.value?.name || "");
    }
    triggerCallback();
  };
  ...
  return (
    <AppCard className={className} noPadding>
      <div className="tw:grid tw:grid-cols-1 tw:gap-4 tw:px-4 tw:py-3 tw:md:grid-cols-6">
        ...
        {/* Employee search temporarily hidden.
        <MasterEmployeeSearchInput
          size="sm"
          label="Employee"
          placeholder="Search employee"
          className="tw:md:col-span-2"
          callback={handleEmployeeSelect}
          values={
            employeeId
              ? [
                  {
