...
const Retailers = () => {
  ...
  const formMethods = useForm<RetailerSummaryFilterForm>({
  ...
  // The header tiles aggregate across every retailer, honouring only the
  // location / search / created-on filters (not pagination).
  ...
  const handleFilterChange = useCallback(
    (formData: RetailerSummaryFilterForm) => {
      ...
      if (
        ...
        params.set(
          "startDate",
          format(formData.followUpDateRange[0], "yyyy-MM-dd"),
        );
        params.set(
          "endDate",
          format(formData.followUpDateRange[1], "yyyy-MM-dd"),
        );
      }
  ...
  useEffect(() => {
    const next: RetailerSummaryFilterForm = { ...defaultFilter };
    FILTER_KEYS.forEach((key) => {
