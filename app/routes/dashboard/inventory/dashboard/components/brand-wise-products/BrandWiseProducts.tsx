...
const BrandWiseProducts = () => {
  ...
  const [data, setData] = useState<BrandValue[]>([]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error loading more brand wise products:", error);
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.error("Error downloading brand wise products:", error);
  ...
  const handleCountClick = (
    ...
    setSearchParams(
      (prev) => {
        ...
        next.set("view", tab);
        if (item.brandRefId) {
          next.set("brandId", item.brandRefId);
          next.set("brandName", item.brandName || "");
        } else {
          next.delete("brandId");
          next.delete("brandName");
        }
        // Clear brand-wise specific filters that don't apply to the
        // sku movement list, plus any lingering menu/category filters from a
