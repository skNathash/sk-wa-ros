...
/**
 * A single category row in the subscribe browse-by-category accordion.
 * Collapsed it shows the category thumb, name and a "N deals · M brands"
 * subtitle; expanding reveals the category's top subscribable deals via
 * <CategoryDeals>. Mirrors the inventory browse-category `CategoryBlock`, but
 * sourced from the subscribe category feed.
 */
const CategoryBlock = ({
  category,
  ...
  // The category feed's `totalDeals` is a looser aggregate and reads higher
  // than the subscribe list; once the panel has fetched the count under the
  // list's own filter, that number replaces it in the subtitle.
  const [resolvedDeals, setResolvedDeals] = useState<number | null>(null);
  const dealsCount = resolvedDeals ?? category.totalDeals ?? 0;
  const brandsCount = category.uniqueBrandCount || 0;
  ...
  const subtitle = [
    `${dealsCount} ${t("deals", { defaultValue: "deals" })}`,
    brandsCount ? `${brandsCount} ${t("brands", { defaultValue: "brands" })}` : "",
  ]
    .filter(Boolean)
    .join(" · ");
        ...
        >
          <div className="tw:overflow-hidden">
            <CategoryDeals
              ...
              onTotalResolved={setResolvedDeals}
            />
          </div>
        </div>
      ) : null}
