...
const Category = ({
  ...
  const [selectedAlpha, setSelectedAlpha] = useState(selectedBrand ? "" : "A");
  const [sortType, setSortType] = useState("popular");
  const filterRef = useRef<Record<string, any>>({
    alpha: selectedBrand ? "" : "A",
  ...
  useEffect(() => {
    // Update filterRef with selectedBrand
    filterRef.current = {
      ...filterRef.current,
      search: "",
      brand: selectedBrand,
    ...
    // Reset alpha selection when brand is selected
    if (selectedBrand) {
      setSelectedAlpha("");
      filterRef.current = {
        ...filterRef.current,
