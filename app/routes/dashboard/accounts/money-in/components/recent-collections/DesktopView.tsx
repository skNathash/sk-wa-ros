...
export const detailRows = (item: Collection) => (
  <div className="tw:grid tw:gap-1.5 tw:bg-gray-50 tw:px-4 tw:py-2 tw:md:grid-cols-2">
    ...
    <div className="tw:flex tw:items-center tw:gap-2 tw:text-[11px]">
      <span className="tw:text-gray-500">Received by</span>
      <span className="tw:truncate tw:font-semibold tw:text-gray-700">
        {item.receivedBy}
      </span>
    </div>
...
const DesktopView = ({ data, expanded, toggle }: DesktopViewProps) => {
  /* Nothing collected in the range — the card already carries the white, so
     the empty read stands on its own without the table chrome. */
        ...
        {data.map((item) => (
          <Fragment key={item.id}>
            ...
            >
              ...
              <AppTable.Cell className="tw:text-right tw:text-sm tw:font-bold tw:text-emerald-700">
