...
const BoxOverview: React.FC<BoxOverviewProps> = ({ box }) => {
  if (!box) {
    return (
  ...
  return (
    <>
      <div className="tw:text-sm tw:text-gray-500 tw:mb-2">
        Select boxes to receive. Items in selected boxes will be automatically
        allocated to warehouse locations.
      </div>
        ...
        <div>
          {box.items?.map((item: any, index: number) => (
            ...
            >
              ...
              <div className="tw:bg-gray-100 tw:text-gray-900 tw:px-3 tw:py-1 tw:rounded-full tw:text-sm tw:font-medium tw:ml-4">
                {item.qty || 0} units
              </div>
            </div>
          ))}
