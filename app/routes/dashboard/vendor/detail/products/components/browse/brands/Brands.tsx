...
const Brands = ({
  ...
  const applyFilter = useCallback(async () => {
    // if no category provided, don't attempt to load brands
  ...
  const handleItemClick = (item: any) => {
    if (callback) callback({ action: "view", data: item });
  };

  return (
    <AppCard
      title={`Brands (${paginationRef.current.totalRecords})`}
