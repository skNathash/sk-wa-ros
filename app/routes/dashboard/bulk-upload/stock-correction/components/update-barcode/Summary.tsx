...
const Summary: React.FC<SummaryProps> = ({ valid, invalid, total }) => {
  return (
    /* Paired on mobile, one row of three from sm up — same as the pricing
       preview's stats row. */
    <div className="tw:grid tw:grid-cols-2 tw:sm:grid-cols-3 tw:gap-2 tw:sm:gap-3 tw:mb-4">
      ...
      {/* Odd card out on the two-up mobile grid, so it takes the full row. The
          span lives on a wrapper — AppStatsCard's own `className` goes to its
          inner div, not to the grid item. */}
