...
const UpdateBarcode = ({ info }: UpdateBarcodeProps) => {
  ...
  const methods = useForm<BarcodeFormValues>({
  ...
  const getUploadUrl = () => {
    return API + "purchase/inventory/excel/stockmaster-correction-upload?correctionType=no-barcode";
  };
  ...
  const validate = (records: BarcodeRecord[]): { msg: string } | null => {
    for (const r of records) {
      if (!r.barcode) {
        return {
          msg: `Please enter Barcode for Row ${r.slNo} - ${r.dealName || r.dealRefId}.`,
        };
      }
      if (r.barcode.trim().length < 3) {
        return {
          msg: `Barcode must have at least 3 characters for Row ${r.slNo} - ${r.dealName || r.dealRefId}.`,
        };
      }
    }
    return null;
  ...
  const doSubmit = async () => {
    ...
    if (response.statusCode === 200) {
      ...
      if (errorsList.length === 0) {
        ...
        // Update records with error status for failed items
    ...
    return (
      <FormProvider {...methods}>
        ...
        <AppAlertDialog
          ...
          description={`Are you sure you want to submit the barcode corrections? Only valid records (${computedSummary.valid} of ${computedSummary.total}) will be considered for submission.`}
          onConfirm={doSubmit}
