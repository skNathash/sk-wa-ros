...
export interface MrpFormValues {
  records: MrpRecord[];
}
...
const UpdateMrp = () => {
  ...
  const methods = useForm<MrpFormValues>({
  ...
  const getUploadUrl = () => {
    return API + "purchase/inventory/excel/stockmaster-correction-upload?correctionType=update-mrp";
  };
    ...
    if (response.statusCode === 200) {
      ...
      reset({
        records: sorted.map((rec: any, index: number) => ({
          ...
          oldMrp: rec.oldMrp || rec.mrp || 0,
          mrp: rec.mrp || 0,
          barcode: Array.isArray(rec.barcode)
            ? rec.barcode[0] || ""
            : rec.barcode || "",
          remarks: rec.remarks || "",
          status: rec.status || "",
  ...
  const validate = (records: MrpRecord[]): { msg: string } | null => {
    for (const r of records) {
      if (!r.mrp || r.mrp <= 0) {
        return {
          msg: `Please enter New MRP for Row ${r.slNo} - ${r.dealName || r.dealRefId}.`,
        };
      }
      if (r.purchasePrice > 0 && r.mrp < r.purchasePrice) {
        return {
          msg: `New MRP should not be less than Purchase Price for Row ${r.slNo} - ${r.dealName || r.dealRefId}.`,
        };
      }
      if (r.oldMrp > 0 && r.mrp === r.oldMrp) {
        return {
          msg: `Old MRP and New MRP are same for Row ${r.slNo} - ${r.dealName || r.dealRefId}. Please enter a different New MRP.`,
        };
      }
    }
    return null;
    ...
    return (
      <FormProvider {...methods}>
        ...
        <AppAlertDialog
          ...
          description={`Are you sure you want to submit the MRP updates? Only valid records (${computedSummary.valid} of ${computedSummary.total}) will be considered for submission.`}
          onConfirm={doSubmit}
