...
import type { MenuItem } from "./MenuListPanel";
...
/** Pastel tile tints cycled per card (shown behind the menu image/initial). */
const TILE_TINTS = [
  "tw:bg-emerald-50",
  "tw:bg-rose-50",
  "tw:bg-sky-50",
...
/**
 * Theme-2 "Browse by menu" view — replaces the desktop 3-column
 * menu/category/product layout with a card grid (see MenuGrid for the mobile
 * variant). Each card links to the network product list filtered by that menu.
 */
const MenuBrowseGrid = ({
  items,
  loading,
  loadingMore,
  hasMoreData,
      ...
      <div className="tw:grid tw:grid-cols-2 tw:sm:grid-cols-3 tw:lg:grid-cols-5 tw:gap-2 tw:md:gap-4">
        {items.map((it, idx) => {
          ...
          return (
            <AppLink
              key={`${it._id}-${idx}`}
              href={`/products/buy-from-other-retailer/products/list?menuId=${it._id}&menuName=${encodeURIComponent(it.name)}&distance=${distance}`}
              asLink={true}
              noUnderline
            ...
            >
              {/* Tinted hero — floating white tile with the menu image/initial. */}
              <div
