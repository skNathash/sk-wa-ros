...
const ConfirmModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const totalEnabled = products.filter(
      (p: any) => p?.formData?.isPromotionalDeal === "yes",
    ).length;
    const totalDisabled = products.filter(
      (p: any) => p?.formData?.isPromotionalDeal === "no",
    ).length;
  ...
  return (
    ...
    >
      <AppModal.Title onClose={() => callback && callback({ action: "close" })}>
        Confirm Promotional Configuration
      </AppModal.Title>
        ...
        <div className="tw:bg-blue-50 tw:border tw:border-blue-200 tw:rounded-lg tw:p-3">
          <div className="tw:flex tw:gap-2">
            ...
            <div className="tw:text-sm tw:text-gray-700">
              Review and confirm to process promotional configuration.
            </div>
          </div>
        </div>
      </AppModal.Content>
