...
const ProductSelectCartPromotionalDeal = () => {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    SellerCatalogService.getInventorySourceBreadcrumb(source) || {
      ...
      redirect: {
        path: `/configs/product-select?feature=PromotionDealUpdate${
          source ? `&source=${source}` : ""
        }`,
      },
    },
    { label: "Promotional Deal Config" },
  ];
        ...
        orderBy(cartData?.data || [], "dealInfo.dealName", "asc").map(
          (e: any) => {
            ...
            return {
              ...
              formData: {
                isPromotionalDeal:
                  dealInfo?.isPromotionalDeal === true ||
                  dealInfo?.promotionalConfig?.isActive === true
                    ? "yes"
                    : "no",
              },
            };
    ...
    if (action === "clear-cart") {
      ...
      try {
        ...
        if (response.statusCode === 200) {
          ...
          appNav.replace(
            `/configs/product-select?feature=PromotionDealUpdate${
              source ? `&source=${source}` : ""
            }`,
          );
        } else {
  ...
  const handleConfirmModal = async (args: { action: string; data?: any }) => {
    if (args.action === "submit") {
      ...
      const payload = {
        ...
        items: products.map((e: any) => {
          return {
            ...
            isPromotionalDeal: e.formData.isPromotionalDeal === "yes",
          };
        }),
      };
      ...
      setBusyLoader({
        show: true,
        msg: "Processing promotional configuration...",
      });
        ...
        if (Array.isArray(failures) && failures.length > 0) {
          ...
          setSuccessModal({
            ...
            description:
              ...
              "Promotional configuration processed successfully",
          });
        }
      } else {
        appToast.show({
          msg:
            response.data?.message ||
            "Failed to process promotional configuration",
          color: "danger",
        });
      }
    ...
    if (args.action === "submit") {
      ...
      const updated = (products || []).map((p: any) => {
        return {
          ...
          formData: {
            ...p.formData,
            isPromotionalDeal: data.isPromotionalDeal,
          },
        };
      });
...
export default ProductSelectCartPromotionalDeal;
