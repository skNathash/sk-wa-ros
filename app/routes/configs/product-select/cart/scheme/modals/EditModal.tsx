...
const EditModal = ({ show, callback, data }: Props) => {
  ...
  useEffect(() => {
    if (show && data) {
      ...
      reset({
        offerDiscount: fd.offerDiscount ?? null,
        offerStartDate: fd.offerStartDate ?? null,
        offerEndDate: fd.offerEndDate ?? null,
        schemePrice: fd.schemePrice ?? 0,
        profit: fd.profit ?? 0,
        calculateOn: fd.calculateOn ?? "afterTax",
      });
    }
    if (!show) {
      reset();
  ...
  const onSubmit = () => {
    ...
    const validation = validateForm({
      offerDiscount: values.offerDiscount ?? 0,
      offerStartDate: values.offerStartDate ?? null,
      offerEndDate: values.offerEndDate ?? null,
      schemePrice: values.schemePrice ?? 0,
      profit: values.profit ?? 0,
      calculateOn: values.calculateOn ?? "afterTax",
    });
        ...
        <FormProvider {...formMethods}>
          ...
          >
            <PriceForm
              ...
              additionalCess={data?.dealInfo?.additionalCess}
            />
          </form>
        </FormProvider>
      </AppModal.Content>
