class PrintPutaway {
  static printPutaway(
    ...
    options: { boxId: string; invoiceNo: string; orderId: string }
  ) {
    let items = "";

    // Filter location details for locationName = 'Sellable'
    const filteredLocations = locationDetails.filter(
      (location: any) => location.locationName === "Sellable"
    );

    // Generate table rows for products
    products.forEach((e: any, i: number) => {
      // Find the corresponding location details for each product
      const locationDetails =
        filteredLocations.find((loc: any) => loc.dealId === e.dealId) || {};

      items += `<tr>
        <td class="text-center">${i + 1}</td>
        <td class="product-name">${e.dealName}</td>
        <td class="text-right">Rs.${e.packageMrp}</td>
        <td class="text-center">${e.packageQuantity}</td>
        <td>${locationDetails.locationName || "N/A"}</td>
        <td>${locationDetails.locationRackName || "N/A"}</td>
        <td>${locationDetails.locationBinName || "N/A"}</td>
        <td>${e.barcodeStr || "-"}</td>
      </tr>`;
    });
          ...
          <div class="info-container">
              <div class="info-box">
                  <div class="info-item">
                      ...
                      <span class="info-value">${options.invoiceNo}</span>
                  </div>
                  <div class="info-item">
